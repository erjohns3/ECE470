...
// and change all pixels in each object to the color.
{
	...
	int worldX[maxBlocks];
	int worldY[maxBlocks];
	int worldZ[maxBlocks];
	int sumR[maxBlocks];
	int sumC[maxBlocks];
	...
	int prevRow = -1;
	int prevCol = -1;
	int min = 195;										//------------------need to change-------------------
	int max = 245;										//------------------need to change-------------------
	int numBlocks = 0;
	int rootFinder;

	// camera collaboration
	float originR = height/2;
	float originC = width/2;
	float beta = 3.828;
	float theta = -1.571 - 0.1974;		// radians might be negative
	float transformX = 7.5;
	float transformY = 17;
	...
	*/
		...
		for(int col=0; col<width; col++){
			if(pixellabel[row][col]==0){
				...
				if(prevRow>0 && prevCol>0){
					if(prevRow<prevCol && prevCol!=-1){
						equiv[prevCol] = prevRow;
					}else if(prevCol<prevRow && prevRow!=-1){
	...
	for(int i=0; i<numBlocks; i++){
		...
		worldX[i] = -1*(((centerC[i]-originC)/beta)*cos(theta) + (((height-centerR[i])-originR)/beta)*sin(theta)) + transformX;
		worldY[i] = -1*((((height-centerR[i])-originR)/beta)*cos(theta) - ((centerC[i]-originC)/beta)*sin(theta)) + transformY;
		worldZ[i] = 3;
	}
	...
	console_printf("number of blocks: %i, |RED| c:%d,%d w:%d,%d,%d |GREEN| c:%d,%d w:%d,%d,%d |BLUE| c:%d,%d w:%d,%d,%d", numBlocks, centerR[0], centerC[0], worldX[0], worldY[0], worldZ[0], centerR[1], centerC[1], worldX[1], worldY[1], worldZ[1], centerR[2], centerC[2], worldX[2], worldY[2], worldZ[2]);
